    ...
    <div className="min-h-screen bg-[#0B0F1A] text-white flex flex-col">
      ...
      <nav className="flex items-center justify-between px-6 sm:px-12 py-5 border-b border-[#2A3352]">
        <div className="flex items-center gap-3">
          ...
          <span className="font-head font-extrabold text-lg tracking-tight">
            Ring<span className="text-[#F5A623]">Vault</span>
          </span>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/auth/login"
            className="text-[13px] text-[#8B92B0] hover:text-white font-semibold transition-colors px-3 py-1.5">
            Sign In
          </Link>
          <Link href="/auth/signup"
            className="text-[13px] bg-[#F5A623] hover:bg-[#E8891A] text-black font-bold px-4 py-2 rounded-lg transition-all">
            Get Started
          </Link>
        </div>
      </nav>
      ...
      <section className="flex-1 flex flex-col items-center justify-center text-center px-4 py-20">
        ...
        <h1 className="font-head font-extrabold text-4xl sm:text-5xl md:text-6xl leading-tight mb-6 max-w-3xl">
          ...
          <span className="text-[#F5A623]">SMS Verification</span>
        </h1>

        <p className="text-[#8B92B0] text-base sm:text-lg max-w-xl mb-10 leading-relaxed">
          Buy or rent real virtual phone numbers from the USA and worldwide. 
          Receive OTP codes instantly for any app — WhatsApp, Telegram, Google, and more.
        </p>

        <div className="flex flex-col sm:flex-row gap-4 items-center">
          <Link href="/auth/signup"
            className="px-8 py-3.5 bg-[#F5A623] hover:bg-[#E8891A] text-black font-head font-bold text-base rounded-xl transition-all shadow-lg shadow-[#F5A623]/20">
            Start for Free →
          </Link>
          <Link href="/auth/login"
            className="px-8 py-3.5 bg-[#1C2236] hover:bg-[#242B42] border border-[#2A3352] text-white font-semibold text-base rounded-xl transition-all">
            Sign In
          </Link>
        </div>
      </section>
