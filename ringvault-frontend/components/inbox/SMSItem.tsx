...
function extractLink(text: string | undefined): string | null {
  if (!text) return null
  const slashes = String.fromCharCode(47, 47)
  const pattern = new RegExp('https?:' + slashes + '\\S+')
...
export function SMSItem({ msg }: { msg: SMSLog }) {
  ...
  const wrapperClass = clsx(
    ...
    msg.isNew ? 'border-[#F5A623] bg-[rgba(245,166,35,0.05)]' : 'border-[#2A3352] bg-[#1C2236]'
  )

  return (
    <div className={wrapperClass}>
      <div className="w-10 h-10 rounded-xl bg-[rgba(79,142,247,0.12)] border border-[rgba(79,142,247,0.2)] grid place-items-center text-lg flex-shrink-0">
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            {msg.isNew && (
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[rgba(245,166,35,0.15)] text-[#F5A623]">NEW</span>
            )}
          </div>
        ...
        {!showFull && <div className="text-[13px] mt-1 line-clamp-2">{msg.body}</div>}
        ...
        {showFull && (
          <div className="mt-3 bg-[#0B0F1A] border border-[#2A3352] rounded-lg p-3">
            ...
            {link && (
              <a href={link} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 text-[12px] text-[#4F8EF7] hover:underline break-all">
                🔗 {link}
              </a>
            )}
          </div>
