...
export function Sidebar({ user }: { user: any }) {
  ...
  return (
    <>
      {/* DESKTOP DRAWERS SIDEBAR (Visible on screens >= 768px) */}
      <aside className="hidden md:flex w-[220px] min-w-[220px] bg-[#131826] border-r border-[#2A3352] flex-col py-5 h-screen sticky top-0">
        {/* Logo */}
      ...
      {/* MOBILE NAV TRAY (Visible on screens < 768px) */}
      <nav className="flex md:hidden fixed bottom-0 left-0 right-0 h-16 bg-[#131826] border-t border-[#2A3352] z-50 items-center justify-around px-2 pb-safe">
        {NAV.map(item => {
          const active = path === item.href || (item.href !== '/dashboard' && path.startsWith(item.href))
          return (
            <Link key={item.href} href={item.href} className={clsx(
              'flex flex-col items-center justify-center relative w-16 h-full transition-all gap-1',
              active ? 'text-[#F5A623]' : 'text-[#8B92B0]'
            )}>
              {/* Highlight bar directly above current selection */}
              {active && <span className="absolute top-0 left-1/4 right-1/4 h-[3px] bg-[#F5A623] rounded-b" />}
              
              <span className="text-xl leading-none">{item.icon}</span>
              <span className="text-[10px] font-medium tracking-tight truncate max-w-full">{item.label.split(' ')[0]}</span>
              
              {/* Context Floating Notification Badge */}
              {item.badge && (
                <span className="absolute top-1 right-2 bg-[#F5A623] text-black text-[9px] font-bold w-4 h-4 grid place-items-center rounded-full">
                  {item.badge}
                </span>
              )}
            </Link>
